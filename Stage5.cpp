...
void Stage5::Initialize()
{
	...
	// コマンド
	LoadData("resource/csv/boxData5.csv", boxPopComands_);
...
void Stage5::LoadData(const std::string& filename, std::stringstream& targetStream)
{
	std::ifstream file;
	// ファイルを開く
...
void Stage5::Update(char* keys, char* preKeys)
{
	// シーン遷移演出
	inScene_->Update();
...
void Stage5::AddBox(Vector2 pos, Vector2 size)
{
	// 弾の生成
	Box* obj = new Box();
	// 初期化
...
void Stage5::AddMetalBox(Vector2 pos, Vector2 size)
{
	// 弾の生成
...
void Stage5::AddIceBox(Vector2 pos, Vector2 size)
{
	// 弾の生成
...
void Stage5::AddTvBox(Vector2 pos, Vector2 size)
{
	// 弾の生成
	TvBox* obj = new TvBox();
	// 初期化
...
void Stage5::AddHitEffect(uint32_t texture, uint32_t anim, uint32_t animMax,
...
void Stage5::UpdateBoxComands(std::stringstream& boxPopComands)
{
	// 1行分の文字列を入れる変数
	std::string line;
...
void Stage5::Retry()
{
	...
	// CSVファイルを再度読み込む
	std::string csvFilePath = "resource/csv/boxData5.csv";
