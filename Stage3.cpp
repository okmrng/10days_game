...
void Stage3::Initialize()
{
	...
	// コマンド
	LoadData("resource/csv/boxData3.csv", boxPopComands_);
...
void Stage3::LoadData(const std::string& filename, std::stringstream& targetStream)
{
	std::ifstream file;
	// ファイルを開く
...
void Stage3::AddBox(Vector2 pos, Vector2 size)
{
	// 弾の生成
	Box* obj = new Box();
	// 初期化
...
void Stage3::AddMetalBox(Vector2 pos, Vector2 size)
{
	// 弾の生成
...
void Stage3::AddIceBox(Vector2 pos, Vector2 size)
{
	// 弾の生成
...
void Stage3::AddTvBox(Vector2 pos, Vector2 size)
{
	// 弾の生成
	TvBox* obj = new TvBox();
	// 初期化
...
void Stage3::AddHitEffect(uint32_t texture, uint32_t anim, uint32_t animMax,
...
void Stage3::UpdateBoxComands(std::stringstream& boxPopComands)
{
	// 1行分の文字列を入れる変数
	std::string line;
...
void Stage3::Retry()
{
	...
	// CSVファイルを再度読み込む
	std::string csvFilePath = "resource/csv/boxData3.csv";
